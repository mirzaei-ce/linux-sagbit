Sagbit version 0.5.0 is now available for download at:
http://sourceforge.net/projects/sagbit/files/Sagbit/sagbit-0.5.0/
...
For Ubuntu users, there is a new ppa maintained by Matt Corallo which you can add to your system so that it will automatically keep sagbit up-to-date.  Just type "sudo apt-add-repository ppa:sagbit/sagbit" in your terminal, then install the sagbit-qt package.
...
If you have a previously encrypted wallet.dat, the first time you run sagbit-qt or sagbitd the wallet will be rewritten, Sagbit will
...
Sagbit-Qt: Address Book, then New Address...
...
Drag and drop support for sagbit: URIs on web pages.
...
signmessage/verifymessage : new RPC commands to sign a message with one of your private keys or verify that a message signed by the private key associated with a sagbit address.
