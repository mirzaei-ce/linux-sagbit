Sagbit Core version 0.11.0 is now available from:

  <https://sagbit.org/bin/sagbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Sagbit Core or other software:
...
For example, add the following to `sagbit.conf`:
...
related to the blockchain in the sagbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Sagbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Sagbit Core will 
...
For an example script see [sagbit-submittx](https://github.com/laanwj/sagbit-submittx).
...
- #5334 `cf87536` libsagbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new sagbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only SagbitAmountField
- #5753 `0f44672` Add sagbit logo to about screen
...
- #5809 `46bfbe7` Add sagbit-cli man page
...
- #5849 `41259ca` contrib/init/sagbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Sagbit-Qt.app to Sagbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for sagbit-tx)
