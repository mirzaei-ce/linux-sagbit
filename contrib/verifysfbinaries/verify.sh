...
###   This script attempts to download the signature file SHA256SUMS.asc from sagbit.org
...
WORKINGDIR="/tmp/sagbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://sagbit.org/bin/"
VERSIONPREFIX="sagbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sagbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Sagbit Core binary release signing key installed?"
   fi
